...
        }
    } else {
        console.warn("AVISO: Validação de segurança do webhook desativada.");
    }
    
...
                        
                        // ALTERAÇÃO AQUI: Apenas atualiza se o pagamento foi aprovado
                        if (pedidoDoBanco.status !== novoStatus && novoStatus === 'PAGO') {
...
// FUNÇÕES AUXILIARES DE E-MAIL E MELHOR ENVIO...
...
            from: `"Carlton" <${EMAIL_USER}>`, to: EMAIL_TO, bcc: pedido.email_cliente,
            subject: `Confirmação do Pedido #${pedido.id}`, html: emailBody,
        });
...
    try {
        await transporter.sendMail({
            from: `"Carlton" <${EMAIL_USER}>`, to: pedido.email_cliente,
            subject: `Código de Rastreio - Pedido #${pedido.id}`, html: emailBody,
        });
...
    // ... (código existente sem alterações) ...
    const emailBody = `<h1>⚠️ Pagamento não confirmado para o Pedido #${pedido.id}</h1><p>Olá, ${pedido.nome_cliente}.</p><p>Notamos que o pagamento referente ao seu pedido <strong>#${pedido.id}</strong> ainda não foi confirmado.</p><p>O link para pagamento via PIX expirou para evitar pagamentos duplicados ou fraudes. Se ainda deseja adquirir os produtos, por favor, realize um novo pedido em nosso site.</p><p>Se você já pagou e acha que isso foi um engano, por favor, entre em contato conosco com o comprovante de pagamento.</p><hr><p>Agradecemos a sua compreensão.</p><p>Atenciosamente,</p><p>Equipe Carlton</p>`;

    try {
        await transporter.sendMail({
            from: `"Carlton" <${EMAIL_USER}>`, to: pedido.email_cliente,
            subject: `Aviso: Pagamento Pendente para o Pedido #${pedido.id}`, html: emailBody,
        });
...
        
        // ALTERADO: Adicionado metodo_pagamento e final_cartao na busca
        const sql = `
            SELECT 
...
            if (methodId && methodId.includes('card')) return 'Cartão de Crédito';
            if (methodId === 'pix') return 'PIX';
            return methodId || 'Não informado';
        };
...
            
            // ALTERADO: Agora usa os dados do banco
            pagamento: {
                metodo: getPaymentMethodName(pedidoDoBanco.metodo_pagamento),
                final_cartao: pedidoDoBanco.final_cartao
            },
